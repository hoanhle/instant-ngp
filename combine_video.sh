...
if [ $DRAW_TEXT == "ON" ]; then
    BASE_FILTER="
        $BASE_FILTER;\
        [base]drawtext=text='instant-ngp (baseline)':fontcolor=white:fontsize=h/50:x=w/50:y=h/50[base];\
        [ours]drawtext=text='ours':fontcolor=white:fontsize=h/50:x=w-tw-w/50:y=h/50[ours]"
fi
if [ $DRAW_BAR == "ON" ]; then
    BASE_FILTER="
        $BASE_FILTER;\
